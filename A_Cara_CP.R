...
#Criando mapas
g_brasil <- plot_mapa(doc_total, "") #GRAFICO 10
...
#Grafico proporcao de mulheres por tipo de programa (GRAFICO 1)
...
#Plotando modelo de valores preditos de probabilidade de ser orientado por masculino por sexo (GRAFICO 2)
...
#Gerando nuvem de palavras (GRAFICO 15)
...
#Grafico - Redes de principais orientadores (GRAFICO 4)
...
#Salvar grafico para o apendice (Grafico 16)
png("diagnostico_numero_topicos.png", width = 3000, height = 2200, res = 300)
plot(k_result)
dev.off()

#COMPARAÇÃO VISUAL DOS TÓPICOS (K = 7 a 13) (Quadro 2)

Ks_compare <- 7:13
models <- vector("list", length(Ks_compare))
names(models) <- as.character(Ks_compare)

#Rodar e guardar modelos
for(k in Ks_compare){
  set.seed(1234)
...
#n de palavras por topico
n_words <- 7

png("Figura_A2_comparacao_topicos_K7aK13.png",
    width = 4000,
    height = 9000,
    res = 300)

par(mfrow = c(length(Ks_compare), 1),
    mar = c(4,4,3,1))

for(k in Ks_compare){
  plot(models[[as.character(k)]],
       n = n_words,
       labeltype = "frex",
       main = paste("K =", k))
}
...
#Plotando topicos mais influentes (Gráfico 17)
plot(model.stm, n=5, labeltype="frex")
png("figura4_topicos_stm.png", width = 3000, height = 2200, res = 300)
plot(model.stm, n = 5, labeltype = "frex")
dev.off()

#Olhando topico por topico para classificar (Quadro 3)
...
#Grafico de calor de topico (GRAFICO 5)
...
topics <- paste0("topic_", 1:10)

#Padroniza DV (tópicos) e contínuas (z = (x - média)/DP)
Data_std <- Data %>%
  mutate(
    across(all_of(topics), ~ as.numeric(scale(.x))),
    Nota = as.numeric(scale(Nota)),
    Ano_Obtencao = as.numeric(scale(Ano_Obtencao))
  )

#Modelos com DV padronizada + contínuas padronizadas
models_std <- lapply(topics, function(t){
  formula <- as.formula(
    paste(t, "~ Tipo + Regiao + Nota + Sexo + Pais + Sexo_O + Ano_Obtencao")
  )
  lm(formula, data = Data_std)
})
names(models_std) <- topics

#Outputs com SE robusto HC3 + IC95%
results <- lapply(names(models_std), function(n){
...
#Significância por IC95%
results <- results %>%
  mutate(sig = ifelse(conf.low > 0 | conf.high < 0, "significativo", "ns"))
...
results$modelo <- factor(
  results$modelo,
  levels = paste0("topic_", 1:10),
  labels = labels_topics
)

results <- results %>%
  mutate(term = case_when(
    term == "TipoHISTÓRIA" ~ "História",
    term == "TipoADMINISTRAÇÃO PÚBLICA" ~ "Administração Pública",
    term == "TipoPOLÍTICAS PÚBLICAS" ~ "Políticas Públicas",
    term == "TipoSOCIOLOGIA" ~ "Ciências Sociais",
    
    term == "RegiaoCentro-Oeste" ~ "Centro-Oeste",
    term == "RegiaoSul" ~ "Sul",
    term == "RegiaoNordeste" ~ "Nordeste",
    term == "RegiaoNorte" ~ "Norte",
    
    term == "Nota" ~ "Nota",
    term == "SexoMasculino" ~ "Masculino",
    term == "PaisEstados Unidos" ~ "Estados Unidos",
    term == "PaisEuropa" ~ "Europa",
    term == "Sexo_OMasculino" ~ "Orientador Masculino",
    term == "Ano_Obtencao" ~ "Ano de Obtenção PhD",
    TRUE ~ term
  ))
...
coef_plot <- ggplot(
  results,
  aes(x = estimate, y = term, color = modelo, alpha = sig)
) +
  ...
  scale_alpha_manual(values = c("significativo" = 1, "ns" = 0.25), guide = "none") +
  ...
  labs(
    x = "Coeficiente padronizado (DP) com IC95% (SE robusto HC3)",
    y = "",
    color = "Tópico"
  ) +
...
ggsave(
  filename = "coef_plot_topicos_padronizados_HC3.svg",
  plot = coef_plot,
...
tab_ms <- results %>%
  mutate(
    stars = stars_fun(p.value),
    cell  = sprintf("%.3f%s\n(%.3f)", estimate, stars, std.error)
  ) %>%
  select(term, modelo, cell) %>%
  pivot_wider(
    names_from  = modelo,
    values_from = cell
  ) %>%
...
writeData(wb, "Modelsummary", tab_ms)

setColWidths(wb, "Modelsummary", cols = 1:ncol(tab_ms), widths = "auto")
addStyle(
  wb, "Modelsummary",
  style = createStyle(halign = "center", valign = "center", wrapText = TRUE),
  rows = 1:(nrow(tab_ms) + 1), cols = 2:ncol(tab_ms),
  gridExpand = TRUE
)
addStyle(
  wb, "Modelsummary",
  style = createStyle(textDecoration = "bold", halign = "center"),
  rows = 1, cols = 1:ncol(tab_ms),
  gridExpand = TRUE
)

saveWorkbook(wb, "tabela1.xlsx", overwrite = TRUE)
...
impact_vars <- c("CitAno","Cit5Ano","h","h5","i10Ano","i105Ano")
topics <- paste0("topic_", 1:10)

#Padronizar impacto (DV), tópicos (IV) e contínuas
Data_std2 <- Data %>%
  mutate(
    across(all_of(impact_vars), ~ as.numeric(scale(.x))),
    across(all_of(topics), ~ as.numeric(scale(.x))),
    Nota = as.numeric(scale(Nota)),
    Ano_Obtencao = as.numeric(scale(Ano_Obtencao))
  )

#Rodar 6 modelos (topic_10 RI omitido como baseline)

impact_models <- lapply(impact_vars, function(v){
  
  f <- as.formula(
    paste(
      v,
      "~ Tipo + Regiao + Nota + Sexo + Pais + Sexo_O + Ano_Obtencao +",
      paste(topics[-10], collapse = " + ")
    )
  )
  
  lm(f, data = Data_std2)
})
names(impact_models) <- impact_vars

#SE robusto HC3 + IC95%

impact_results <- lapply(names(impact_models), function(n){
...
impact_results$modelo <- factor(
  impact_results$modelo,
  levels = impact_vars,
  labels = c(
    "Citações/Ano",
    "Citações 5 anos/Ano",
...
labels_topics <- c(
  "Tópico: Direito",
  "Tópico: Comportamento e Instituições",
  "Tópico: Saúde e Meio Ambiente",
  "Tópico: Administração e Gestão Pública",
  "Tópico: História do Pensamento Político",
  "Tópico: Ciências Sociais",
  "Tópico: Educação",
  "Tópico: Economia Política",
  "Tópico: Teoria Democrática e Participação",
  "Tópico: Relações Internacionais"
)

topic_map <- setNames(labels_topics[1:9], paste0("topic_", 1:9))

impact_results <- impact_results %>%
  mutate(term = case_when(
    term == "TipoPOLÍTICAS PÚBLICAS" ~ "Tipo: Políticas Públicas",
    term == "TipoSOCIOLOGIA" ~ "Tipo: Sociologia",
    term == "TipoADMINISTRAÇÃO PÚBLICA" ~ "Tipo: Administração Pública",
    term == "TipoHISTÓRIA" ~ "Tipo: História",
    
    term == "RegiaoNordeste" ~ "Região: Nordeste",
    term == "RegiaoCentro-Oeste" ~ "Região: Centro-Oeste",
    term == "RegiaoSul" ~ "Região: Sul",
    term == "RegiaoNorte" ~ "Região: Norte",
    
    term == "Nota" ~ "Nota do Programa",
    term == "SexoMasculino" ~ "Masculino",
    
    term == "PaisEstados Unidos" ~ "Doutorado: Estados Unidos",
    term == "PaisEuropa" ~ "Doutorado: Europa",
    
    term == "Sexo_OMasculino" ~ "Orientador Masculino",
    term == "Ano_Obtencao" ~ "Ano de Obtenção do PhD",
    
    term %in% names(topic_map) ~ unname(topic_map[term]),
    
    TRUE ~ term
  ))

term_levels_final <- c(
  "Tipo: Políticas Públicas",
  "Tipo: Sociologia",
  ...
  "Nota do Programa",
  "Masculino",
  "Doutorado: Estados Unidos",
  "Doutorado: Europa",
  "Orientador Masculino",
  "Ano de Obtenção do PhD",
  
  labels_topics[1:9]
)

impact_results$term <- factor(impact_results$term, levels = term_levels_final)

write_csv(impact_results, "outputs_impacto_padronizados_HC3.csv")

impact_plot_all <- ggplot(
  impact_results,
  aes(x = estimate, y = term, color = modelo, alpha = sig)
) +
  ...
  scale_alpha_manual(values = c("significativo" = 1, "ns" = 0.25), guide = "none") +
  scale_color_manual(values = c(
    "Citações/Ano" = "darkblue",         
    "Citações 5 anos/Ano" = "darkred", 
    "h-index" = "darkgreen",             
    "h5-index" = "#5B21B6",           
    "i10/Ano" = "darkorange",             
    "i10 5 anos/Ano" = "black"       
  )) +
  labs(
    x = "Coeficiente padronizado (DP) com IC95% (SE robusto HC3)",
    y = "",
    color = "Indicador de Impacto"
  ) +
...
ggsave(
  filename = "coef_plot_impacto_todos_termos_padronizados_HC3.svg",
  plot = impact_plot_all,
  device = svglite::svglite,
  width = 13,
  height = 13,
  units = "in"
...
addWorksheet(wb, "Impacto")

writeData(wb, "Impacto", impact_table)

setColWidths(wb, "Impacto", cols = 1:ncol(impact_table), widths = "auto")

addStyle(
  wb, "Impacto",
  style = createStyle(halign = "center", valign = "center", wrapText = TRUE),
  rows = 1:(nrow(impact_table) + 1),
  cols = 2:ncol(impact_table),
  gridExpand = TRUE
)
...
summary(matching) #Tabela 3

plot(matching, type = "jitter") #Grafico 31
...
#Grafico com resultados do matching (Grafico 8)
...
#meu computador ainda e de estudante de pos-grad :)
...
#Sensibilidade matching (Tabela 4)

outcomes <- c("CitAno", "Cit5Ano", "i10Ano", "i105Ano", "h", "h5")

rosenbaum_results <- map(outcomes, function(out){
  
  pair_diff <- matching_data %>%
    filter(!is.na(subclass)) %>%
    group_by(subclass) %>%
    summarise(
      y1 = .data[[out]][Sexo_mat == 1],
      y0 = .data[[out]][Sexo_mat == 0],
      diff = y1 - y0,
      .groups = "drop"
    ) %>%
    pull(diff)
  
  pair_diff <- pair_diff[!is.na(pair_diff)]
  
  ps <- psens(
    x = abs(pair_diff),
    y = as.integer(pair_diff > 0),
    Gamma = 3,
    GammaInc = 0.05
  )
  ...
  list(
    outcome = out,
    n_pairs = length(pair_diff),
    psens = ps
  )
  
})

names(rosenbaum_results) <- outcomes
...
term_map <- c(
  "TipoPOLÍTICAS PÚBLICAS"     = "Tipo: Políticas Públicas",
  "TipoSOCIOLOGIA"             = "Tipo: Sociologia",
  "TipoADMINISTRAÇÃO PÚBLICA"  = "Tipo: Administração Pública",
  "TipoHISTÓRIA"               = "Tipo: História",
  
  "RegiaoNordeste"             = "Região: Nordeste",
  "RegiaoCentro-Oeste"         = "Região: Centro-Oeste",
  "RegiaoSul"                  = "Região: Sul",
  "RegiaoNorte"                = "Região: Norte",
  
  "Nota"                       = "Nota do Programa",
  "PaisEstados Unidos"         = "Doutorado: Estados Unidos",
  "PaisEuropa"                 = "Doutorado: Europa",
  "Sexo_OMasculino"                     = "Orientador Masculino",
  "Ano_Obtencao"               = "Ano de Obtenção do PhD",
  
  "topic_1"                    = labels_topics[1],
  "topic_2"                    = labels_topics[2],
  "topic_3"                    = labels_topics[3],
  "topic_4"                    = labels_topics[4],
  "topic_5"                    = labels_topics[5],
  "topic_6"                    = labels_topics[6],
  "topic_7"                    = labels_topics[7],
  "topic_8"                    = labels_topics[8],
  "topic_9"                    = labels_topics[9]
)

#Modelo (padronização + HC3)

md_std <- md_all %>%
  mutate(
    Ano_Obtencao = scale(Ano_Obtencao)[,1],
    Nota         = scale(Nota)[,1],
    topic_1      = scale(topic_1)[,1],
    topic_2      = scale(topic_2)[,1],
    topic_3      = scale(topic_3)[,1],
    topic_4      = scale(topic_4)[,1],
    topic_5      = scale(topic_5)[,1],
    topic_6      = scale(topic_6)[,1],
    topic_7      = scale(topic_7)[,1],
    topic_8      = scale(topic_8)[,1],
    topic_9      = scale(topic_9)[,1]
  )

Privilegio <- lm(
  matched ~ Tipo + Regiao + Nota + Pais + Sexo_O + Ano_Obtencao +
    topic_1 + topic_2 + topic_3 + topic_4 + topic_5 + topic_6 +
    topic_7 + topic_8 + topic_9,
  data = md_std
)

V <- vcovHC(Privilegio, type = "HC3")

results <- broom::tidy(lmtest::coeftest(Privilegio, vcov = V)) %>%
  mutate(
    conf.low  = estimate - 1.96 * std.error,
    conf.high = estimate + 1.96 * std.error,
    sig95     = ifelse(p.value < 0.05, "Significativo (p<0,05)", "Não significativo")
  )
...
plot_df <- results %>%
  ...
  # remove qualquer coisa que não esteja mapeada (evita NA no eixo)
  filter(!is.na(term_label)) %>%
  mutate(
    term_label = factor(term_label, levels = term_levels_final)
  )

coef_plot <- ggplot(plot_df, aes(x = estimate, y = term_label, color = sig95)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey40") +
  geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = .2, linewidth = 0.6) +
  geom_point(size = 2.8) +
  scale_color_manual(
    values = c(
      "Significativo (p<0,05)" = "black",
      "Não significativo" = "darkgrey"
    )
  ) +
  labs(
    x = "Coeficiente padronizado (DP) com IC95% (SE robusto HC3)",
...
priv_table <- plot_df %>%  
  ...
  select(term = term_label, modelo, cell) %>%
  mutate(
    term = factor(term, levels = term_levels_final)
  ) %>%
  arrange(term) %>%
  mutate(term = as.character(term)) %>%
  pivot_wider(
...
wb <- createWorkbook()
addWorksheet(wb, "Privilegio")

writeData(wb, "Privilegio", priv_table)

setColWidths(wb, "Privilegio", cols = 1:ncol(priv_table), widths = "auto")

addStyle(
  wb, "Privilegio",
  style = createStyle(halign = "center", valign = "center", wrapText = TRUE),
  rows = 1:(nrow(priv_table) + 1),
  cols = 2:ncol(priv_table),
  gridExpand = TRUE
)
